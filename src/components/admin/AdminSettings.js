...
// Build canonical item name from a course name + raw schedule option string.
// "Pieles Perfectas" + "Opción 1: 2PM A 4PM" → "Pieles Perfectas 2PM a 4PM"
function buildItemName(courseName, scheduleOption) {
  const m = scheduleOption.match(/(\d+)\s*([AP]M)\s+[Aa]\s+(\d+)\s*([AP]M)/i);
  if (!m) return null;
  const [, h1, p1, h2, p2] = m;
  return `${courseName} ${h1}${p1.toUpperCase()} a ${h2}${p2.toUpperCase()}`;
}
...
export default function AdminSettings() {
  ...
  const seatConfig = useMemo(() => {
    ...
    for (const course of courseSettings) {
      ...
      // courses. Fall back to buildItemName only for admin-created courses that
      // don't have a canonical entry yet.
      const canonicalNames = courseCanonicalItems[course.courseId];
      const itemNames = canonicalNames
        || schedOpts.map(opt => buildItemName(course.courseName, opt)).filter(Boolean);
